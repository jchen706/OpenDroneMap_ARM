...
load_file (std::string const& filename)
{
	...
	throw; }
		...
		std::cout << "Exception in jpeg in image io" << std::endl;
	}
#endif
	...
	return load_tiff_file(filename); 
		...
		std::cout << "Exception in tiff in image io" << std::endl;
	}
#endif
...
save_file (ByteImage::ConstPtr image, std::string const& filename)
{
    ...
    std::cout << "save image name image io " << filename << std::endl;
#ifndef MVE_NO_JPEG_SUPPORT
...
struct tiffErrMgr {
        jmp_buf setjmp_buffer_tiff;
	char msg[2048];
};

tiffErrMgr tiff_err_manager;
